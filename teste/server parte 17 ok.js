    ...
    ` : ""}
      ...
      <div class="box">
        <form class="busca" method="GET" action="/painel">
          ...
          <select name="ordem">
            <option value="desc" ${filtroOrdem === "desc" ? "selected" : ""}>Mais recentes</option>
            <option value="asc" ${filtroOrdem === "asc" ? "selected" : ""}>Mais antigas</option>
          </select>
          <input
            type="date"
            name="data_inicio"
            value="${escapeHtml(filtroDataInicio)}"
          />
          <input
            type="date"
            name="data_fim"
            value="${escapeHtml(filtroDataFim)}"
          />
...
app.get("/exportar-csv", middlewareProtegePainel, async (req, res) => {
  try {
    ...
    const query = `
      ...
      ${whereBase}
      ORDER BY criado_em ${ordem.toUpperCase()}
    `;
