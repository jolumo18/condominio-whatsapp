...
function renderizarPainelHtml(
  ...
  const linhas = reclamacoes.map((r) => {
    ...
    const statusValor = r.status || "-";
    const status = escapeHtml(statusValor);
    ...
    return `
      <tr>
        ...
        <td>
          <span style="${estilosStatus[statusValor] || 'padding:6px 10px;border-radius:999px;display:inline-block;font-weight:bold;background:#f1f1f1;color:#333;'}">
            ${status}
          </span>
        </td>
        <td>${criadoEm}</td>
        ...
        <table>
          ...
          <div class="box">
            <div style="display:flex; gap:8px; flex-wrap:wrap;">
              ${paginaAtual > 1 ? `<a class="btn-limpar" href="/painel?protocolo=${encodeURIComponent(filtroProtocolo)}&telefone=${encodeURIComponent(filtroTelefone)}&nome=${encodeURIComponent(filtroNome)}&categoria=${encodeURIComponent(filtroCategoria)}&status=${encodeURIComponent(filtroStatus)}&pagina=${paginaAtual - 1}">Página anterior</a>` : ""}
              <span style="padding:10px 14px;">Página ${paginaAtual} de ${totalPaginas}</span>
              ${paginaAtual < totalPaginas ? `<a class="btn-limpar" href="/painel?protocolo=${encodeURIComponent(filtroProtocolo)}&telefone=${encodeURIComponent(filtroTelefone)}&nome=${encodeURIComponent(filtroNome)}&categoria=${encodeURIComponent(filtroCategoria)}&status=${encodeURIComponent(filtroStatus)}&pagina=${paginaAtual + 1}">Próxima página</a>` : ""}
            </div>
          </div>
        </body>
        </html>
...
app.get("/exportar-csv", middlewareProtegePainel, async (_req, res) => {
  try {
    const { rows } = await pool.query(`
      SELECT protocolo, nome_contato, telefone, categoria, bloco, unidade, descricao, status, criado_em
      FROM reclamacoes
      ORDER BY criado_em DESC
    `);
...
app.get("/painel", middlewareProtegePainel, async (req, res) => {
  try {
    ...
    if (condicoes.length > 0) {
      whereBase += ` WHERE ${condicoes.join(" AND ")} `;
    }
    ...
    const valoresDados = [...values, limite, offset];
...
app.post("/painel/status", middlewareProtegePainel, async (req, res) => {
  try {
    ...
    const observacao = normalizarTexto(req.body.observacao || "");
